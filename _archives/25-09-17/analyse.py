...
# Parameters
max_bin = 5000    # max bin distance in µm
num_bins = 500    # number of bins between 0 and max bin
...
def analyse(img_path, max_bin=1000, num_bins=100):
    ...
    # Analyse
    bins, values, baseline = _analyse(
        img, edt, metadata["pixel_size (µm)"], metadata["df"])
    ...
    io.imsave(
        dir_path / "image_bsub.tif", (img - baseline).astype("float32"), 
