...
class Correct:
    ...
    def init_images(self):        
        ...
        for img_path in self.img_paths:
            ...
            if msk_hc_path.exists():
                self.msks_hc.append(io.imread(msk_hc_path))
                self.pnts.append(io.imread(pnt_path))
                self.outs.append(io.imread(out_path))
            else:
                self.msks_hc.append(np.zeros_like(msk))
    ...
    def init_viewer(self):
        ...
        self.btn_revert_all = QPushButton("Revert mask")
        ...
        act_group_layout.addWidget(self.btn_revert_all)
        ...
        self.btn_revert_all.clicked.connect(self.revert_all)
...
#%% Functions(s) --------------------------------------------------------------
    ...
    def get_outline(self, msk_hc, pnt):
        msk_hc = msk_hc > 0
        pnt = pnt > 0
    ...
    def save_mask(self):
        ...
        pnt = self.viewer.layers["pnt"].data
        if np.max(label(pnt)) != 2:
            raise ValueError("The mask was not saved, please check pnt layer")
        out = self.get_outline(msk_hc, pnt)
        self.viewer.layers["out"].data = out
        self.msks_hc[self.idx] = msk_hc
        self.outs[self.idx] = out
        
        io.imsave(
            str(self.img_paths[self.idx]).replace("img", "msk_hc"),
            msk_hc.astype("uint8"), check_contrast=False,
            )
        io.imsave(
            str(self.img_paths[self.idx]).replace("img", "pnt"),
            pnt.astype("uint8"), check_contrast=False,
            )
        io.imsave(
            str(self.img_paths[self.idx]).replace("img", "out"),
            out, check_contrast=False,
            )
        
    def revert_all(self):
        self.msks_hc[self.idx] = np.zeros_like(self.msks[self.idx])
        self.pnts[self.idx] = np.zeros_like(self.msks[self.idx])
        self.outs[self.idx] = np.zeros_like(self.msks[self.idx])
        ...
        Path(str(self.img_paths[self.idx]).replace("img", "msk_hc")).unlink()
        Path(str(self.img_paths[self.idx]).replace("img", "pnt")).unlink()
        Path(str(self.img_paths[self.idx]).replace("img", "out")).unlink()
...
#%% development ---------------------------------------------------------------
    ...
    # for props in regionprops(label(tmp_out)):
    #     coords = props.coords
    #     vals = label(pnt_hc)[tuple(coords.T)]
    #     if len(np.unique(vals)) != 3:
    #         out[tuple(coords.T)] = 0
    
    # # Display
    # vwr = napari.Viewer()
    # vwr.add_image(msk_hc,  visible=1, colormap="gray",   opacity=0.25)
    # vwr.add_image(pnt_hc,  visible=1, colormap="yellow", blending="additive")
    # vwr.add_image(out,     visible=1, colormap="gray",   blending="additive")
